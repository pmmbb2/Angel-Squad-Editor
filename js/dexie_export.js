...
var campaign = assetsDir + 'campaigns/'+ document.querySelector('#cmpFileName').value + '.json'		
			...
			*/
			if(ext != '.json'){
				...
				*/ 
				...
				load(assetsDir + selectedCollection + '/'+assetName+ext, function(jsonAsset){
...
function populateCmpSelector(selectorId){
    ...
    document.addEventListener('click', function (event) {
		...
		if (event.target.matches('.selectCmp')) {
			...
			// SET THE EXPORT FILE NAME BY DEFAULT
			var fileNameArray = nameWoExt.split('.')
				fileNameArray.pop()// remove '.cmp'
				nameWoExt = fileNameArray.join('.')
				nameWoExt += '.exp'
  ...
  exportLink.onclick = async(event)=>{	
    ...
    spawn(function* (){	
		...
		browser.downloads.download({
		 url: URL.createObjectURL(new Blob([ JSON.stringify(tableObjects) ])),
		...
		*/
		...
		//var fileName = 'myFirstCampaign.exp.json';
		  ...
		  .then(function(rawResponse){			  
			  ...
			  var msg = (rawResponse.statusText == 'OK')?'exp file '+tableObjects.fileName+' created successfully!':'error'
			  alert(msg)
		  })	
